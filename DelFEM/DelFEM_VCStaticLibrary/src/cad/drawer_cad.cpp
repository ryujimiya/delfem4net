  ...
  indAry_old.clear();
	...
	{	// 頂点をセット
		...
		for(unsigned int iver=0;iver<aVertex.size();iver++){
			...
			// added by ryujimiya
			double cd[3];
			cad_2d.GetColor_Vertex(id_v_cad, cd);
			dpv.color[0] = cd[0];
			dpv.color[1] = cd[1];
			dpv.color[2] = cd[2];
			// endof added
...
void CDrawer_Cad2D::Draw() const
{
  ...
  { // draw vertecies
    ...
    for(unsigned int iver=0;iver<this->m_aIndexVertex.size();iver++){
      ...
      // deleted by ryujimiya
      //else{ ::glColor3d(0.0,0.0,0.0);	}
      // endof deleted
      // added by ryujimiya
      else{ ::glColor3fv(this->m_aIndexVertex[iver].color); }
      // endof added
