...
// added by ryujimiya
bool CCadObj2D::GetColor_Vertex(unsigned int id_v, double color[3]) const
{
  if (!m_BRep.IsElemID(Cad::VERTEX,id_v)) return false;
...
// added by ryujimiya
bool CCadObj2D::SetColor_Vertex(unsigned int id_v, const double color[3])
{
  if (!m_BRep.IsElemID(Cad::VERTEX,id_v)) return false;
  if (!m_VertexSet.IsObjID(id_v)) return false;
  CVertex2D& v = m_VertexSet.GetObj(id_v);
  v.m_color[0] = color[0];
  v.m_color[1] = color[1];
  v.m_color[2] = color[2];
  return true;
...
// added by ryujimiya
bool CCadObj2D::GetColor_Edge(unsigned int id_e, double color[3]) const
{
  if (!m_BRep.IsElemID(Cad::EDGE,id_e)) return false;
...
// added by ryujimiya
bool CCadObj2D::SetColor_Edge(unsigned int id_e, const double color[3])
{
  if (!m_BRep.IsElemID(Cad::EDGE,id_e)) return false;
  if (!m_EdgeSet.IsObjID(id_e)) return false;
  CEdge2D& e = m_EdgeSet.GetObj(id_e);
...
//			assert( nv>0 ); assert( ne>0 ); assert( nl>0 );
    ...
    for(int iv=0;iv<nv;iv++){
      ...
      CVertex2D v(CVector2D(x,y));
			v.m_color[0] = c[0]; v.m_color[1] = c[1]; v.m_color[2] = c[2];
			int tmp_id = m_VertexSet.AddObj( std::make_pair(id,v) );
			// endof added
			//int tmp_id = m_VertexSet.AddObj( std::make_pair(id,CVertex2D(CVector2D(x,y) )) );  // deleted by ryujimiiya
			assert(tmp_id==id);
			// endof added
		}
    for(int ie=0;ie<ne;ie++){
      ...
      double c[3];  arch.Get("%lf%lf%lf",&c[0],&c[1],&c[2]); // added by ryujimiya
    ...
    { // print Vertex2D
			...
			for(unsigned int iid=0;iid<id_ary.size();iid++){
				...
				arch.Out("%d\n",id_v);
				arch.Out("%lf %lf\n",v.point.x,v.point.y);
        arch.Out("%lf %lf %lf\n",v.m_color[0],v.m_color[1],v.m_color[2]); // added by ryujimiya
			}
		}
    { // print Edge2D
