...
//		const unsigned int ncrs_size = crs.size;
	...
	// modified by ryujimiya
	//  この関数CZMatDia_BlkCrs::AddPatternを同じWorldに対して複数回実行するとパターンが欠落することがある。reserveを取ると安定する？
	//tmp_row_ptr.reserve(crs.Size()+this->NCrs()); // deleted by ryujimiya
