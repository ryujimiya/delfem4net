...
public:
  ...
  id_v_s(rhs.id_v_s), id_v_e(rhs.id_v_e), po_s(rhs.po_s), po_e(rhs.po_e){
    ...
    m_color[0] = rhs.m_color[0]; m_color[1] = rhs.m_color[1]; m_color[2] = rhs.m_color[2]; // added by ryujimiya
  }
  CEdge2D() : id_v_s(0), id_v_e(0), itype(CURVE_LINE){
    m_color[0] = 0.0; m_color[1] = 0.0; m_color[2] = 0.0; // added by ryujimiya
  }
  ...
  inline void GetColor(double color[3]) const {
    color[0] = m_color[0]; color[1] = m_color[1]; color[2] = m_color[2];
  }
  
  inline void SetColor(const double color[3]) {
...
private:
  double m_color[3];  // 辺の色 (added by ryujimiya)
private:
//public:
...
public:
  ...
  double m_color[3];  // 頂点の色 (added by ryujimiya)
};
