...
namespace wg2d
{
    ...
    class WgUtil
    {
        ...
        /// <returns></returns>
        public static bool SolvePortWaveguideEigen(
            double waveLength,
            WaveModeDV waveModeDv,
            CFieldWorld world,
            uint fieldValId,
            ...
            bool isSkipSolveEigen = false)
        {
            ...
            txx_1d = new double[node_cnt, node_cnt]; // 吸収境界条件で使用
            //double[,] uzz_1d = new double[node_cnt, node_cnt];
            uzz_1d = new double[node_cnt, node_cnt]; // 吸収境界条件で使用
            // ryy_1dマトリクス (1次線要素)
        ...
        /// <param name="world">ワールド座標系</param>
        /// <param name="fieldValId">フィールド値ID</param>
        public static void SetFieldValueForDisplay(CFieldWorld world, uint fieldValId, double[] values_all, Dictionary<uint, uint> toNodeIndex)
        {
            Complex[] c_values_all = new Complex[values_all.Length];
            for (int ino = 0; ino < values_all.Length; ino++)
            {
                c_values_all[ino] = new Complex(values_all[ino], 0.0);
            }
            SetFieldValueForDisplay(world, fieldValId, c_values_all, toNodeIndex);
        }
