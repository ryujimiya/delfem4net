...
namespace wg2d
{
    ...
    /// 周期構造導波路解析のユーティリティ
    /// </summary>
    class WgUtilForPeriodicEigenBetaSpecified : WgUtil
    {
        ////////////////////////////////////////////////////////////////////////
        // 型
        ////////////////////////////////////////////////////////////////////////
        ...
        private static bool MkPeriodicHelmholtzMat_EachElementAry(
            ...
            uint free_node_cnt,
            Dictionary<uint, int> toSorted,
            ref KrdLab.clapack.Complex[] KMat,
            ref KrdLab.clapack.Complex[] MMat)
        {
            ...
            // 要素剛性行列
            Complex[,] eKMat = new Complex[nno, nno];
            // 要素質量行列
            Complex[,] eMMat = new Complex[nno, nno];
            ...
            for (uint ielem = 0; ielem < ea.Size(); ielem++)
            {
                ...
                Complex betaWork = new Complex(betam.Real, betam.Imaginary);
                for (int ino = 0; ino < nno; ino++)
                {
                    for (int jno = 0; jno < nno; jno++)
                    {
                        ...
                        if (isYDirectionPeriodic)
                        {
                            eKMat[ino, jno] = media.P[0, 0] * integralDLDY[ino, jno] + media.P[1, 1] * integralDLDX[ino, jno]
                                                 - imagOne * betaWork * media.P[1, 1] * (integralDLDYL[ino, jno] - integralDLDYL[jno, ino])
                                                 + betaWork * betaWork * media.P[1, 1] * integralL[ino, jno];
                        }
                        else
                        {
                            eKMat[ino, jno] = media.P[0, 0] * integralDLDY[ino, jno] + media.P[1, 1] * integralDLDX[ino, jno]
                                                 - imagOne * betaWork * media.P[1, 1] * (integralDLDXL[ino, jno] - integralDLDXL[jno, ino])
                                                 + betaWork * betaWork * media.P[1, 1] * integralL[ino, jno];
                        }
                        // 要素質量行列
                        eMMat[ino, jno] = media.Q[2, 2] * integralL[ino, jno];
                    }
                }
                // 要素剛性行列にマージする
                for (int ino = 0; ino < nno; ino++)
                {
                    ...
                    for (int jno = 0; jno < nno; jno++)
                    {
                        ...
                        // clapack形式の行列格納方法で格納
                        KMat[inoGlobal + free_node_cnt * jnoGlobal] += new KrdLab.clapack.Complex(eKMat[ino, jno].Real, eKMat[ino, jno].Imag);
                        MMat[inoGlobal + free_node_cnt * jnoGlobal] += new KrdLab.clapack.Complex(eMMat[ino, jno].Real, eMMat[ino, jno].Imag);
                    }
                }
            }
