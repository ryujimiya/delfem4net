...
namespace wg2d_periodic_eigen_k
{
    class Problem03_2
    {
            ...
            )
        {
            ...
            // 大きいロッドの半径
            //double rodRadiusRatio_1stRow = rodRadiusRatio;
            //double rodRadiusRatio_1stRow = 0.375;
            double rodRadiusRatio_1stRow = 0.400;
            // ロッドの比誘電率
            double rodEps = 3.476 * 3.476; // for latticeTheta = 60 r = 0.32a air hole n = 3.476
            // 1格子当たりの分割点の数
            const int ndivForOneLattice = 9;
            // ロッド円周の分割数
            //const int rodCircleDiv = 12; // for latticeTheta = 60 r = 0.32a
            ...
            // 大きいロッドの半径
            double rodRadius_1stRow = rodRadiusRatio_1stRow * latticeA;
            // メッシュのサイズ
            ...
            IList<uint> HalfRodLoopIds = new List<uint>();  // check用
            IList<uint> HalfRodEIds = new List<uint>(); // check用
            int ndivPlus = 0;
            ...
            using (CCadObj2D cad2d = new CCadObj2D())
            {
                ...
                if (isLargeRod
                    && ((isShift180 && (rodCntHalf % 2 == 1)) || (!isShift180 && (rodCntHalf % 2 == 0))))
                {
                    int minus = outofAreaRodPtCnt_row_top >= 1 ? (outofAreaRodPtCnt_row_top - 1) : 0;
                    for (int k = 0; k < (rodRadiusDiv * 2 - minus); k++)
                    {
                        HalfRodEIds.Add(id_e_rod_B1[k]);
                        HalfRodEIds.Add(id_e_rod_B1[id_e_rod_B1.Count - 1 - k]);
                        HalfRodEIds.Add(id_e_rod_B2[k]);
                        HalfRodEIds.Add(id_e_rod_B2[id_e_rod_B2.Count - 1 - k]);
                    }
                }
                ...
                for (int col = 1; col <= periodCnt * 2 - 1; col++)
                {
                    ...
                    for (int i = 0; i < rodCntHalf; i++)
                    {
                            ...
                            || (col % 2 == 0 && ((rodCntHalf - 1 - i) % 2 == (isShift180 ? 0 : 1))))
                        {
                            ...
                            uint lId = WgCadUtil.AddRod(cad2d, baseLoopId, x0, y0, rr, rodCircleDiv, rodRadiusDiv);
                            rodLoopIds.Add(lId);
                        }
                    }
                    for (int i = 0; i < rodCntHalf; i++)
                ...
                if (isCadShow)
                {
                    ...
                    // DEBUG
                    foreach (uint eId in HalfRodEIds)
                    {
            ...
            FieldLoopId = 0;
            {
                ...
                for (int i = offset; i < mediaIndex_list.Length; i++)
                {
                    ...
                    // DEBUG
                    //if (HalfRodLoopIds.Contains(rodLoopIds[i - offset]))
                    //{
            ...
            FieldPortBcId1 = 0;
            {
                ...
                if (id_e_rod_B1.Contains(eId_cad_list[0]))
                {
                    ...
                    // DEBUG
                    //if (HalfRodEIds.Contains(eId_cad_list[0]))
                    //{
                    //    mediaIndex_list[0] = Medias.IndexOf(mediaCladding);
                    //}
                }
                ...
                for (int i = 1; i <= ndivPlus - 1; i++)
                {
                    ...
                    if (id_e_rod_B1.Contains(eId_cad_list[i]))
                    {
                        mediaIndex_list[i] = Medias.IndexOf(mediaCore);
                        // DEBUG
                        //if (HalfRodEIds.Contains(eId_cad_list[i]))
                        //{
                        //    mediaIndex_list[i] = Medias.IndexOf(mediaCladding);
                        //}
                    }
