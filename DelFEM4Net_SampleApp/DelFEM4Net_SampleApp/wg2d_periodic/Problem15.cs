...
namespace wg2d_periodic
{
    class Problem15
    {
            ...
            )
        {
            ...
            // ロッドの数(半分)
            //const int rodCntHalf = 3; // for latticeTheta = 60 r = 0.29a air hole
            const int rodCntHalf = 3;
            // 欠陥ロッド数
            const int defectRodCnt = 1;
            // 三角形格子の内角
            double latticeTheta = 60.0; // for latticeTheta = 60 r = 0.29a air hole
            // ロッドの半径
            double rodRadiusRatio = 0.29; // for latticeTheta = 60 r = 0.29a air hole
            // ロッドの比誘電率
            double rodEps = 2.80 * 2.80; // for latticeTheta = 60 r = 0.29a air hole
            // 1格子当たりの分割点の数
            //const int ndivForOneLattice = 9; // for latticeTheta = 60 r = 0.29a air hole
            const int ndivForOneLattice = 9;
            // ロッド円周の分割数
            //const int rodCircleDiv = 12; // for latticeTheta = 60 r = 0.29a
            const int rodCircleDiv = 12;
            // ロッドの半径の分割数
            //const int rodRadiusDiv = 4; // for latticeTheta = 60 r = 0.29a air hole
            const int rodRadiusDiv = 4;
            // 導波路不連続領域の長さ
            const int rodCntDiscon = 3;
            // 共振器の欠陥ロッド数
            const int defectRodCnt_cavity = 3;
            // 共振器と入力導波路の間のロッド数
            const int spacingRodCnt = 2;
            // 最適形状?
            ...
            if (Math.Abs(latticeTheta - 60.0) < MyUtilLib.Matrix.Constants.PrecisionLowerLimit)
            {
                ...
                NormalizedFreq1 = 0.262;
                NormalizedFreq2 = 0.2761;
                FreqDelta = 0.00025;// 0.001;
            ...
            // 計算領域のX方向ロッド数
            int rodCntX = rodCnt_inputWg * 2 + rodCntDiscon * 2 + defectRodCnt_cavity;
            // 計算領域のY方向ロッド数
            int rodCntY = rodCntHalf * 2 + defectRodCnt + spacingRodCnt + rodCntY_cavity;
            // 上端Y座標
            double topY = rodCntY * rodDistanceY;
            // 右端X座標
            double rightX = rodCntX * rodDistanceX;
            ...
            using (CCadObj2D cad2d = new CCadObj2D())
            {
                ...
                //------------------------------------------------------------------
                {
                    ...
                    pts.Add(new CVector2D(rightX - inputWgLength, 0.0)); // 頂点4
                    pts.Add(new CVector2D(rightX, 0.0)); // 頂点5
                    pts.Add(new CVector2D(rightX, WaveguideWidth)); // 頂点6
                    pts.Add(new CVector2D(rightX - inputWgLength, WaveguideWidth)); // 頂点7
                    pts.Add(new CVector2D(rightX - inputWgLength, topY)); // 頂点8
                    pts.Add(new CVector2D(inputWgLength, topY)); // 頂点9
                ...
                for (int col = 1; col <= (periodCntX * 2 - 1); col++)
                {
                    ...
                    // 中央のロッド
                    int inputWg_stRodIndex = (rodCntY - (rodCntHalf * 2 + defectRodCnt));
                    for (int i = 0; i < rodCntY; i++)
                    {
                        // 入出力領域は導波路幅のみ
                        if (col < periodCntInputWg1 * 2 || col >= (periodCntX * 2 - periodCntInputWg1 * 2))
                        {
                            if (i < inputWg_stRodIndex) continue;
                        }
                        // ロッドの位置のオフセット
                        ...
                        // 共振器
                        if (col >= (periodCntInputWg1 * 2 + rodCntDiscon * 2 + 1) && col <= (periodCntInputWg1 * 2 + rodCntDiscon * 2 + defectRodCnt_cavity * 2 - 1))
                        {
                        ...
                        // 共振器の左右のロッドを外方向にずらす
                        double ofs_ratio = 0.00;
                        if (isOpt)
                        {
                            // for rodCntY_cavity == 3
                            //ofs_ratio = 0.00;
                            //ofs_ratio = 0.05;
                            //ofs_ratio = 0.10;
                            ofs_ratio = 0.15;
                            if (i >= (rodCntY - rodCntHalf - defectRodCnt - spacingRodCnt - rodCntY_cavity)
                                && i <= (rodCntY - rodCntHalf - defectRodCnt - spacingRodCnt - 1))
                            {
                                if (col == (periodCntInputWg1 * 2 + rodCntDiscon * 2 - 1))
                                {
                                    ofs_x_rod = -ofs_ratio * latticeA;
                                }
                                else if (col == (periodCntInputWg1 * 2 + rodCntDiscon * 2 + defectRodCnt_cavity * 2 + 1))
                                {
                                    ofs_x_rod = ofs_ratio * latticeA;
                                }
                            }
                            ...
                            || (col % 2 == 0 && (Math.Abs(rodCntY - 1 - i) % 2 == (isShift180 ? 0 : 1))))
                        {
                            ...
                            double y0 = topY - i * rodDistanceY - rodDistanceY * 0.5;
                ...
                for (int i = 0; i <= (rodCntY_F2_cavity - 1); i++)
                {
                    ...
                    {
                        uint id_e = 7;
                        double x0 = rightX - inputWgLength;
                        double y0 = topY - rodDistanceY * i - rodDistanceY * 0.5;
                        double x_cross = rightX - inputWgLength;
                        double[] y_cross_list = new double[3];
