            ...
            )
        {
            ...
            using (CCadObj2D cad2d = new CCadObj2D())
            {
                ...
                //------------------------------------------------------------------
                {
                    ...
                    pts.Add(new CVector2D(inputWgLength, wg2_Y2)); // 頂点5
                    pts.Add(new CVector2D(port2_X - inputWgLength, wg2_Y2)); // 頂点6
                    pts.Add(new CVector2D(port2_X, wg2_Y2)); // 頂点7
                    pts.Add(new CVector2D(port2_X, wg2_Y1)); // 頂点8
                    pts.Add(new CVector2D(port2_X - inputWgLength, wg2_Y1)); // 頂点9
                    pts.Add(new CVector2D(bendX4, wg2_Y1)); // 頂点10
                    pts.Add(new CVector2D(port3_X2_B6, port3_Y2_B6)); // 頂点11
                    pts.Add(new CVector2D(port3_X2_B5, port3_Y2_B5)); // 頂点12
                    pts.Add(new CVector2D(port3_X1_B5, port3_Y1_B5)); // 頂点13
                    pts.Add(new CVector2D(port3_X1_B6, port3_Y1_B6)); // 頂点14
                    pts.Add(new CVector2D(bendX3, bendY3)); // 頂点15
                    pts.Add(new CVector2D(port1_X + inputWgLength, WaveguideWidth));  // 頂点16
                ...
                uint eIdAdd3 = cad2d.ConnectVertex_Line(11, 14).id_e_add;
                    ...
                    for (int i = 0; i < rodCntHalf; i++)
                    {
                        if ((rodCntHalf - 1 - i) % 2 == (isShift180 ? 0 : 1))
                        {
                            ...
                            // 左のロッド
                            {
                                ...
                                {
                                    y0 = WaveguideWidth - i * rodDistanceY - rodDistanceY * 0.5;
                                }
                                else
                                {
                                    y0 = wg2_Y1 - i * rodDistanceY - rodDistanceY * 0.5;
                                }
                    ...
                    for (int i = 0; i < rodCntHalf; i++)
                    {
                        if (i % 2 == (isShift180 ? 0 : 1))
                        {
                            ...
                            // 左のロッド
                            {
                                ...
                                if (colIndex == 0 || colIndex == 1)
                                {
                                    y0 = 0.0 + rodDistanceY * rodCntHalf - i * rodDistanceY - rodDistanceY * 0.5;
                                }
                                else
                                {
                                    y0 = wg2_Y2 + rodDistanceY * rodCntHalf - i * rodDistanceY - rodDistanceY * 0.5;
                                }
                                uint work_id_v0 = id_v0;
                                uint work_id_v2 = id_v2;
                                if (isReverse)
                ...
                for (int col = -1; col <= (periodCntX_port3 * 2); col++)
                {
                    ...
                    for (int i = 0; i < rodCnt_BendY; i++)
                    {
                        ...
                        // 欠陥(導波路2)
                        if ((col >= (periodCntInputWg1 + rodCntDiscon) * 2)
                            && (i >= (rodCnt_BendY - 1 - rodCntHalf - defectRodCnt + 1) && i <= (rodCnt_BendY - 1 - rodCntHalf)))
                        {
                            continue;
                        }
                        ...
                        double rodRadius_Small = 0.16 * latticeA; // ロッド２つ
                ...
                for (int i = (rodCnt_BendY - 1); i >= (rodCnt_BendY - rodCntY_F1_Bend); i--)
                {
                    // 左の強制境界と交差するロッド
                    bool isRodLattice = false;
                    isRodLattice = ((i - (rodCnt_BendY - rodCntY_F1_Bend)) % 2 == 1);
                    if (isRodLattice)
                    {
                        uint id_e = 4;
                ...
                for (int i = (rodCnt_BendY - rodCntY_F1_Bend); i < rodCnt_BendY; i++)
                {
                    // 不連続領域
                    uint baseLoopId = 2;
                    // 左の強制境界と交差するロッド
                    ...
                    {
                        {
                            ...
                            double x0 = inputWgLength;
                            double y0 = WaveguideWidth - rodDistanceY * i - rodDistanceY * 0.5;
                            int row2 = (rodCntY_F1_Bend - 1 - (i - (rodCnt_BendY - rodCntY_F1_Bend))) / 2;
            ...
            {
                uint[] eId_cad_list = new uint[13 + id_e_F1.Count + id_e_F2.Count + id_e_F1_Bend.Count];
                eId_cad_list[0] = 2;
                eId_cad_list[1] = 3;
                eId_cad_list[2] = 4;
                eId_cad_list[3] = 5;
                ...
                {
                    eId_cad_list[13 + i] = id_e_F1[i];
                }
                for (int i = 0; i < id_e_F2.Count; i++)
                {
                    eId_cad_list[13 + id_e_F1.Count + i] = id_e_F2[i];
                }
                for (int i = 0; i < id_e_F1_Bend.Count; i++)
                {
                    eId_cad_list[13 + id_e_F1.Count + id_e_F2.Count + i] = id_e_F1_Bend[i];
                }
            ...
            for (int portIndex = 0; portIndex < portCnt; portIndex++)
            {
                ...
                for (int i = 1; i <= ndivPlus - 1; i++)
                {
                    if (portIndex == 0)
                    {
                        eId_cad_list[i] = (uint)(19 + (ndivPlus_port1 - 1) - (i - 1));
                    }
                    else if (portIndex == 1)
                    {
                        eId_cad_list[i] = (uint)(19 + (ndivPlus_port1 - 1) * 2 + (ndivPlus_port2 - 1) - (i - 1));
                    }
                    else if (portIndex == 2)
                    {
                        eId_cad_list[i] = (uint)(19 + (ndivPlus_port1 - 1) * 2 + (ndivPlus_port2 - 1) * 2 + (ndivPlus_port3 - 1) - (i - 1));
                    }
                    else
                    {
                        System.Diagnostics.Debug.Assert(false);
            ...
            for (int portIndex = 0; portIndex < portCnt; portIndex++)
            {
                ...
                for (int i = 1; i <= ndivPlus - 1; i++)
                {
                    if (portIndex == 0)
                    {
                        eId_cad_list[i] = (uint)(19 + (ndivPlus_port1 - 1) * 2 - (i - 1));
                    }
                    else if (portIndex == 1)
                    {
                        eId_cad_list[i] = (uint)(19 + (ndivPlus_port1 - 1) * 2 + (ndivPlus_port2 - 1) * 2 - (i - 1));
                    }
                    else if (portIndex == 2)
                    {
                        eId_cad_list[i] = (uint)(19 + (ndivPlus_port1 - 1) * 2 + (ndivPlus_port2 - 1) * 2 + (ndivPlus_port3 - 1) * 2 - (i - 1));
                    }
                    else
                    {
                        System.Diagnostics.Debug.Assert(false);
