...
namespace wg2d_periodic
{
    class Problem02
    {
            ...
            )
        {
            ...
            const int rodCntHalf = 5;
            // 格子の数
            const int latticeCnt = rodCntHalf * 2 + 1;
            // 格子定数
            ...
            using (CCadObj2D cad2d = new CCadObj2D())
            {
                ...
                //------------------------------------------------------------------
                {
                    ...
                    pts.Add(new CVector2D(inputWgLength + disconLength + WaveguideWidth, 0.0)); // 頂点4
                    pts.Add(new CVector2D(inputWgLength + disconLength + WaveguideWidth, WaveguideWidth + disconLength)); // 頂点5
                    pts.Add(new CVector2D(inputWgLength + disconLength + WaveguideWidth, WaveguideWidth + disconLength + inputWgLength)); // 頂点6
                    pts.Add(new CVector2D(inputWgLength + disconLength, WaveguideWidth + disconLength + inputWgLength)); // 頂点7
                    pts.Add(new CVector2D(inputWgLength + disconLength, WaveguideWidth + disconLength)); // 頂点8
                    pts.Add(new CVector2D(inputWgLength + disconLength, WaveguideWidth)); // 頂点9
                ...
                // ポート1側直線導波路のロッドを追加
                {
                    ...
                    int rodCntPort1Side = rodCntInputWg1 + rodCntDiscon;
                ...
                // ポート2側直線導波路のロッドを追加
                {
                    ...
                    int rodCntPort2Side = rodCntInputWg2 + rodCntDiscon;
                    for (int col = 0; col < rodCntPort2Side; col++)
                    {
                        ...
                        else if (col >= rodCntInputWg2 && col < rodCntInputWg2 + rodCntDiscon)
                        {
                            baseLoopId = 2;
                            inputWgNo = 0;
                        }
            ...
            for (int portIndex = 0; portIndex < portCnt; portIndex++)
            {
                ...
                for (int i = 1; i <= ndiv - 1; i++)
                {
                    if (portIndex == 0)
                    {
                        eId_cad_list[i] = (uint)(12 + (ndiv - 1) - (i - 1));
                    }
                    else if (portIndex == 1)
                    {
                        eId_cad_list[i] = (uint)(12 + (ndiv - 1) * 3 - (i - 1));
                    }
                    else
                    {
                        System.Diagnostics.Debug.Assert(false);
            ...
            for (int portIndex = 0; portIndex < portCnt; portIndex++)
            {
                ...
                for (int i = 1; i <= ndiv - 1; i++)
                {
                    if (portIndex == 0)
                    {
                        eId_cad_list[i] = (uint)(12 + (ndiv - 1) * 2 - (i - 1));
                    }
                    else if (portIndex == 1)
                    {
                        eId_cad_list[i] = (uint)(12 + (ndiv - 1) * 4 - (i - 1));
                    }
                    else
                    {
                        System.Diagnostics.Debug.Assert(false);
                ...
                WgUtil.GetLoopCoordList(World, wgPortInfo1.FieldInputWgLoopId, out no_c_all, out to_no_loop, out coord_c_all);
                {
                    ...
                    for (int i = 0; i < no_c_all.Length; i++)
                    {
                        ...
                        if ((portIndex == 0 &&
                            ...
                            || (portIndex == 1 &&
                                    (coord[0] >= ((inputWgLength + disconLength + WaveguideWidth) - latticeA * (rodCntHalf + 1)) && coord[0] <= ((inputWgLength + disconLength + WaveguideWidth) - latticeA * (rodCntHalf))))
                            )
                        {
