...
namespace wg2d
{
    ...
    class WgUtilForTD : WgUtil
    {
        ...
        /// <param name="matPattern"></param>
        /// <returns></returns>
        public static void GetOptBandMatNodes(
            bool[,] matPattern,
            IList<uint> sortedNodes,
            out IList<uint> optNodesGlobal,
            out Dictionary<uint, uint> toOptNodes)
        {
            // バンド幅を縮小する
            ...
            optNodesGlobal = new List<uint>();
            toOptNodes = new Dictionary<uint, uint>();
            foreach (int i in optNodes)
            {
                uint ino = sortedNodes[i];
                optNodesGlobal.Add(ino);
                toOptNodes.Add(ino, (uint)(optNodesGlobal.Count - 1));
            }
        }
