...
namespace wg2d_td_pml
{
    class Problem04
    {
            ...
            )
        {
            ...
            // 導波管不連続領域の長さ
            //double disconLength = 10 * periodicDistance + WaveguideWidth;
            double disconLength = 4 * periodicDistance + WaveguideWidth;
            disconLength += pmlLength; // X方向はポート1PMLのみ
            double disconLengthY = 2 * periodicDistance + WaveguideWidth;
            disconLengthY += pmlLength; // Y方向はポート2PMLのみ
            ...
            double port2PosX = disconLength - WaveguideWidth * 0.5;
            double port2PosY = disconLengthY - pmlLength - 1.0 * periodicDistance;
            ...
            // PML開始位置X座標
            double[] pmlStPosX_list = { pmlLength, (disconLengthY - pmlLength) }; // ポート2はY方向PMLでY座標を格納
            // Y方向PML?
            ...
            eId_cad_src_list[0] = 14;
            for (int i = 0; i < 3 * (2 * rodCntHalf); i++)
            {
                eId_cad_src_list[1 + i] = (uint)(16 + (3 * (2 * rodCntHalf) - 1) - i);
            }
            ...
            using (CCadObj2D cad2d = new CCadObj2D())
            {
                ...
                pts.Add(new CVector2D(disconLength, 0.0)); // 頂点5
                pts.Add(new CVector2D(disconLength, disconLengthY - pmlLength)); // 頂点6
                pts.Add(new CVector2D(disconLength, disconLengthY)); // 頂点7
                pts.Add(new CVector2D(disconLength - WaveguideWidth, disconLengthY)); // 頂点8
                pts.Add(new CVector2D(disconLength - WaveguideWidth, disconLengthY - pmlLength)); // 頂点9
                pts.Add(new CVector2D(disconLength - WaveguideWidth, WaveguideWidth)); // 頂点10
                ...
                for (int col = 0; col < colCntX; col++)
                {
                    ...
                    for (int row = 0; row < (rodCntHalf * 2 + defectRodCnt); row++)
                    {
                        ...
                        {
                            // 高透過率のベンド
                            if (col < (colCntX - rodCntHalf - 1))
                            {
                                // ポート1側
                                if (row >= (rodCntHalf) && row < (rodCntHalf + defectRodCnt)) continue; // 欠陥部
                            }
                            if (col == (colCntX - rodCntHalf - defectRodCnt - 1)
                                && row == (rodCntHalf - 1))
                            {
                                continue;
                            }
                            if (col >= (colCntX - rodCntHalf - defectRodCnt) && col < (colCntX - rodCntHalf))
                            {
                                // ポート2側
                                if (row < rodCntHalf) continue; // 欠陥部
                            }
                        }
                        else
                        {
                            // 直角コーナーベンド
                            if (col < (colCntX - rodCntHalf))
                            {
                                // ポート1側
                                if (row >= (rodCntHalf) && row < (rodCntHalf + defectRodCnt)) continue; // 欠陥部
                            }
                ...
                // ポート2側導波路
                int colCntY_port2 = (int)Math.Round((disconLengthY - WaveguideWidth) / periodicDistance);
                for (int col = 0; col < colCntY_port2; col++)
                {
                    uint baseLoopId = 0;
                    double y1 = disconLengthY - col * periodicDistance;
                    double y2 = disconLengthY - (col + 1) * periodicDistance;
                    for (int row = 0; row < (rodCntHalf * 2 + defectRodCnt); row++)
                    {
                        int pmlWgNo = 0;
                        if (y1 >= (disconLengthY - pmlLength + Constants.PrecisionLowerLimit))
                        {
                            pmlWgNo = 2;
                            baseLoopId = 4;
                        }
                        ...
                        if (row >= (rodCntHalf) && row < (rodCntHalf + defectRodCnt)) continue; // 欠陥部
                        double x0 = disconLength - WaveguideWidth + row * latticeA + latticeA * 0.5;
                        double y0 = disconLengthY - col * periodicDistance - periodicDistance * 0.5;
                        uint lId = WgCadUtil.AddRod(cad2d, baseLoopId, x0, y0, rodRadius, rodCircleDiv, rodRadiusDiv);
                        rodLoopIds.Add(lId);
                        if (pmlWgNo == 1)
                        {
                ...
                if (isCadShow)
                {
                    ...
                    // DEBUG
                    {
                        uint[] work_eId_list = { 2, 3, 4, 5, 6, 8, 9, 10, 11, 12 }; // PML終端磁気壁
                        foreach (uint eId in work_eId_list)
                        {
