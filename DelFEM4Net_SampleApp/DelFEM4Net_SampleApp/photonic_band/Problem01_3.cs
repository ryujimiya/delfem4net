...
namespace photonic_band
{
    class Problem01_3
    {
        ...
        /// <summary>
        /// フォトニック結晶 三角形格子(斜め領域) コーナーに1/4ロッド
        /// </summary>
            ...
            )
        {
            ...
            using (CCadObj2D cad2d = new CCadObj2D())
            {
                ...
                int outofAreaRodPtCnt_B3 = rodRadiusDiv + 1;
                for (int axisIndex = 0; axisIndex < 2; axisIndex++) // X方向、Y方向の意味
                {
                    ...
                    if (axisIndex == 0)
                    {
                        ...
                        cur_periodicDistanceY = periodicDistanceY / Math.Sin(latticeTheta * pi / 180.0);
                        ys = ys_B1;
                        ys_rod = ys_rod_B1;
                    ...
                    if (axisIndex == 0)
                    {
                        cur_outofAreaRodPtCnt = outofAreaRodPtCnt_B1;
                    }
                    else if (axisIndex == 1)
                    {
                        cur_outofAreaRodPtCnt = outofAreaRodPtCnt_B3;
                    }
                    else
                    {
                        System.Diagnostics.Debug.Assert(false);
                    ...
                    for (int boundaryIndex = 0; boundaryIndex < 2;  boundaryIndex++)
                    {
                        ...
                        for (int i = 0; i < yAry.Length; i++)
                        {
                            ...
                            if (axisIndex == 0 && boundaryIndex == 0)
                            {
                                // 境界1：左側
                                id_e = 1;
                                x_pt = yAry[i] * Math.Cos(latticeTheta * pi / 180.0);
                                y_pt = yAry[i] * Math.Sin(latticeTheta * pi / 180.0);
                                yAryIndex = i;
                            ...
                            else if (axisIndex == 0 && boundaryIndex == 1)
                            {
                                // 境界2：右側
                                id_e = 3;
                                x_pt = periodicDistanceX + yAry[yAry.Length - 1 - i] * Math.Cos(latticeTheta * pi / 180.0);
                                y_pt = yAry[yAry.Length - 1 - i] * Math.Sin(latticeTheta * pi / 180.0);
                ...
                System.Diagnostics.Debug.Assert(id_v_list_rod_B1.Count == rodCntY * (rodRadiusDiv * 2 + 1 - outofAreaRodPtCnt_B1));
                System.Diagnostics.Debug.Assert(id_v_list_rod_B2.Count == rodCntY * (rodRadiusDiv * 2 + 1 - outofAreaRodPtCnt_B1));
                System.Diagnostics.Debug.Assert(id_v_list_rod_B3.Count == rodCntY * (rodRadiusDiv * 2 + 1 - outofAreaRodPtCnt_B3));
                System.Diagnostics.Debug.Assert(id_v_list_rod_B4.Count == rodCntY * (rodRadiusDiv * 2 + 1 - outofAreaRodPtCnt_B3));
                ...
                for (int rodIndex = 0; rodIndex < 2; rodIndex++)
                {
                    ...
                    {
                        // 右下
                        int index_v0 = rodRadiusDiv;
                        int index_v2 = rodRadiusDiv - 1;
                        id_v0 = id_v_list_rod_B3[index_v2];
                        id_v1 = id_v_B2_bottom_rod_center;
                        id_v2 = id_v_list_rod_B2[index_v0];
                ...
                for (int rodIndex = 0; rodIndex < 2; rodIndex++)
                {
                    ...
                    else if (rodIndex == 1)
                    {
                        ...
                        int index_v0 = rodRadiusDiv;
                        int index_v2 = rodRadiusDiv - 1;
                        id_v0 = id_v_list_rod_B2[index_v2];
                        id_v1 = id_v_B2_top_rod_center;
                        id_v2 = id_v_list_rod_B4[index_v0];
                        x0 = periodicDistanceX + ofsX;
                        y0 = periodicDistanceY;
