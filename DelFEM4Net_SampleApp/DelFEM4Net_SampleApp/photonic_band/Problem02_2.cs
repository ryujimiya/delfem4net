...
namespace photonic_band
{
    class Problem02_2
    {
        ...
        /// <summary>
        /// フォトニック結晶 三角形格子 コーナーに1/4ロッド + 中央にロッド
        /// </summary>
            ...
            )
        {
            ...
            // 格子定数
            latticeA = 0.5 * periodicDistanceY / Math.Sin(latticeTheta * pi / 180.0);
            // 周期構造距離
            periodicDistanceX = 0.5 * periodicDistanceY * 2.0 / Math.Tan(latticeTheta * pi / 180.0);
            // ロッドの半径
            double rodRadius = 0.30 * latticeA;
            // ロッドの比誘電率
            double rodEps = 2.76 * 2.76;
            // 格子１辺の分割数
            const int ndivForOneLattice = 20;// 16;
            // ロッドの円周分割数
