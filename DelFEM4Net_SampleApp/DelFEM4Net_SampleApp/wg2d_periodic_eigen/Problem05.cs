...
namespace wg2d_periodic_eigen
{
    class Problem05
    {
            ...
            )
        {
            ...
            // 基本モードを計算する
            //CalcModeIndex = 0; // even
            // 高次モードを指定する
            //CalcModeIndex = 1; // odd
            // モード追跡する?
            //IsModeTrace = true;
            //IsModeTrace = false;  // 結合長を求めるときは、モード追跡不要
                    ...
                    || Math.Abs(latticeTheta - 30.0) < MyUtilLib.Matrix.Constants.PrecisionLowerLimit)
            {
                ...
                //NormalizedFreq1 = 0.219;// 0.2185;
                //NormalizedFreq2 = 0.22301;//0.2401;
                //FreqDelta = 0.00025;//0.0005;
                //GraphFreqInterval = 0.001;//0.004;//0.005;
                // for latticeTheta = 30 r = 0.30a air hole n = 2.76
                NormalizedFreq1 = 0.267;
                NormalizedFreq2 = 0.273;//0.2831;
                FreqDelta = 0.0005;//0.001;
                GraphFreqInterval = 0.001;//0.004;
            }
            else
            {
